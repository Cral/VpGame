...
void AVpPawn::Move( float DeltaSeconds )
{
	...
	FVector InputDirection = FVector( 0.f, RightValue, UpValue ).GetClampedToMaxSize( 1.0f );
	FVector MoveDirection = FVector(1.0f, InputDirection.Y, InputDirection.Z);

	//const FVector RotateDirection = FVector( 1.0f, RightValue, UpValue ).GetClampedToMaxSize( 1.0f );

	// Calculate  movement
	FVector Movement = FVector(MoveDirection.X * MoveSpeed, MoveDirection.Y * StrafeSpeed, MoveDirection.Z * ClimbSpeed);
